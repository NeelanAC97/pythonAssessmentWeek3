...
def three(arg1):
    ...
    # Given a string seperate the string into the individual numbers present, then add each digit 
	# of each number to get a final value for each number
...
def six(input):
	...
	for letterindex, letter in input[:-1]:

		if letter == "e" and input[letterindex+1] == "i":
...
def nine(inputString, char):
	...
	for i in inputString:
		...
		if i == char:
			...
			position = inputString.index(i)+1
			break
...
"""def ten(string, number, char):

    for i in string:

        if (string.index(i) < len(string)):
			
			if (string.index(i)+1 == number):
				if (i == char):
					return True
